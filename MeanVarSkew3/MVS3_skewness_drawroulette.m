function [RoulT, RoulS, out_ang, out_amt]  = MVS3_skewness_drawroulette(PARAMETERS, sureamount, input2, gamble_col, choice)
%Roulette display, scanner version. It plays out the gamble like a roulette
%wheel. Amounts have been converted to pounds.
% M Symmonds 16-6-9 - adapted by Michael Moutoussis for NSPN.
...
% RoulT.endspin = time;
[RoulS.endspin , RoulT.endspin ] = lastslice(PARAMETERS.port);

waituntil(RoulT.startout + T_end + texttime3)
% waituntil(RoulT.endspin + texttime3)
%output time and amount
% RoulT.end = time;
[RoulS.end , RoulT.end ] = lastslice(PARAMETERS.port);
