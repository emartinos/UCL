function MVS3_earnings_summary = MVS3b(dataDir, code, age, sex, paramset)
%% BEHAVIOURAL PREFERENCES WITH SCANNING & EMULSCANNING CALLS REMOVED
% Edited for depression genetics project compiled code packages 22 Jan 13
% Usage e.g.:
% testDir = [getenv('HOMEDRIVE') getenv('HOMEPATH') '\sibs_data\detailed']
% MVS3b(testDir, '001', 67, 'm',5)
...
% Original by Mkael Symmonds - adapted by Michael Moutoussis for Depression Genetics study
...
if nargin < 1
  dataDir = [getenv('HOMEDRIVE') getenv('HOMEPATH') '\sibs_data\detailed'];
end
...
switch SUBJECT.run_params
    ...
    case {'s'}
        ...
        if PARAMETERS.port==0 % && paramset==5 % originally: strcmp(PARAMETERS.port, 'net')
            ...
            % start_emulscan
            disp(' MVS3b - no scanning or emulscanning here');
        end
        ...
        for screen_number=1:size(SKEW_list,1)
            ...
            if screen_number==1                  
                ...
                %% lines changed / replaced for MVS3b ~~~~~~~~~~~~~~~~~~
                wait(1000); 
                % was: waitslice(PARAMETERS.port, 1); % to use up any left over large slice numbers from previous runs
                SLICE.start_experiment=0;  T.start_experiment = time;
                % was:  disp('Waiting for dummy slices ...');
                % was: [SLICE.start_experiment T.start_experiment]=waitslice(PARAMETERS.port, PARAMETERS.NUM.slice*PARAMETERS.NUM.dummy +1);  
                
                SLICE.trialstart=0; % for MVS3b ... was:  = SLICE.start_experiment;
                %SLICE.start_experiment is the actual slice number of the start of the
                %experiment(i.e. after the dummy scans)
                % end changed for MVS3b ~~~~~~~~~~~~~
            else
                cgflip(0, 0, 0);                            % show blank screen
                cgdrawsprite (6, 0, 0);                     % fixation cross
                cgflip (0, 0, 0);              %present cross on screen and clear offscreen
                SLICE.trialstart=0;  T.trialstart = time; 
                % was: [SLICE.trialstart T.trialstart] =  lastslice(PARAMETERS.port);
            end
            disp('Waiting for length of ISI');
            % for MVS3b , convert slices back to milliseconds and do timing:
            waituntil(T.trialstart + (PARAMETERS.TIMINGS.ISI + RUNJITTERLIST(screen_number))* PARAMETERS.TIMINGS.slice);
            % was: waitslice(PARAMETERS.port, SLICE.trialstart + PARAMETERS.TIMINGS.ISI + RUNJITTERLIST(screen_number));      %wait for length of ISI
            ...
            if RUNLIST(screen_number,1)>=0                     %if stim is a pie gamble
                ...
                SLICE.stimulusstart =0; T.stimulusstart = time; % for MVS3b
                % was: [SLICE.stimulusstart T.stimulusstart] =  lastslice(PARAMETERS.port);
                               
                % wait until end of presentation time of stimulus
                clearkeys %get keypresses during this time (not meant to press a key)
                waituntil(T.stimulusstart + PARAMETERS.TIMINGS.stimuluson * PARAMETERS.TIMINGS.slice); % for MVS3
                % was: waitslice(PARAMETERS.port, SLICE.stimulusstart + PARAMETERS.TIMINGS.stimuluson);
                ...
                SLICE.cuestart=0; T.cuestart = time; % for MVS3b
                % was: [SLICE.cuestart, T.cuestart] = lastslice(PARAMETERS.port);
                key_selection = 0;
                pressed = 0;
                ...
                if isequal(trial_counter/PARAMETERS.trials_per_block, floor(trial_counter/PARAMETERS.trials_per_block)) %block end when trial_counter/n is an integer
                    ...
                    cgflip(0, 0, 0);                        % show blank screen
                    cgdrawsprite (6, 0, 0);                 % fixation cross
                    SLICE.lasttrialstart=0; T.lasttrialstart = time; % for MVS3b
                    % was: [SLICE.lasttrialstart T.lasttrialstart]= lastslice(PARAMETERS.port);                    %T.trialstart records time that fixation cross is presented
                    cgflip (0, 0, 0);                       %present cross on screen and clear offscreen
                    waituntil(T.lasttrialstart + PARAMETERS.TIMINGS.endblock1 * PARAMETERS.TIMINGS.slice); % for MVS3
                    % was: waitslice(PARAMETERS.port, SLICE.lasttrialstart + PARAMETERS.TIMINGS.endblock1);      %wait for length of ISI
                    cgpencol(1,1,1)
                    % text('Selecting one lottery from block:', 0, 0)
                    SLICE.lasttrialtext1=0;  T.lasttrialtext1 = time; % for MVS3b
                    % was: [SLICE.lasttrialtext1 T.lasttrialtext1] = lastslice(PARAMETERS.port);
                    ...
                    [RoulT, RoulS, out_ang, out_amt] =  MVS3b_skewness_drawroulette(PARAMETERS, current_sureamount, PIE(selected_trial).data, PARAMETERS.gamble.cixf,    DATA.exp_data.numbers(screen_number - (trial_counter - selected_trial),8)); %find the choice made for the selected outcome
                ...
                %% Lines changed for strictly behavioural version: ~~~~~~~
                SLICE.surestart = 0; T.surestart = time; %record start time for target
                % was: [SLICE.surestart, T.surestart] = lastslice(PARAMETERS.port);      %record start time for target
                cgflip(0,0,0)               %present picture on screen
                % wait until end of presentation time of stimulus:
                waituntil(T.surestart + PARAMETERS.TIMINGS.surescreen*PARAMETERS.TIMINGS.slice); 
                % was: waitslice(PARAMETERS.port, SLICE.surestart + PARAMETERS.TIMINGS.surescreen);
                %% end of strictly behav. block ~~~~~~~~~~~~~~~~~~~~~~~~~~
        ...
        if any(KEY.out==abortkey) || any(KEY.time1.out==abortkey)       
            ...
            if PARAMETERS.port==0              
                % stop_emulscan;
                disp('... and no need to stop_emulscan at abortKey');
            end
            stop_cogent
            break
        else
            ...
            %% lines changed for strictly behaviour-only version: ~~~~~~~~
            % was: [SLICE.end_experiment, T.end_experiment] = waitslice(PARAMETERS.port, lastslice(PARAMETERS.port) + (PARAMETERS.NUM.slice*PARAMETERS.NUM.post));  %wait for post volumes
            % replaced by:
            wait(3000);                             %wait 3s
            SLICE.end_experiment=0; T.end_experiment = time;  % for MVS3b
        ...
        DATA.endslice = 0; % for MVS3b... was:  SLICE.end_experiment;
        DATA.endtime=T.end_experiment;
        DATA.totalvolumes = 0; % for MVS3b... was: SLICE.end_experiment/PARAMETERS.NUM.slice;
...
if PARAMETERS.port==0
  ...
  % stop_emulscan;
  disp('... and no need to stop_emulscan at end');
