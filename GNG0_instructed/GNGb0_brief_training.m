function TotalWon = GNGb0_brief_training(code, dataDir, goNoGoDir, debug)
% function TotalWon = GNGb01(code, dataDir, goNoGoDir, debug)
% function for the Go-NoGo experiment after Marc Guitart, adapted for
% behavioural use for the depression genetics study.
% Based on GNGi1, has flow control for brief training version to be run
% before any other versions of GNG.
%
...
% noWinnings = GNGb0_brief_training(['GNGbBT_' datestr(now,'yyyymmddHHMM')],'.',[getenv('HOMEDRIVE') getenv('HOMEPATH') '\Dropbox\task_code\GNG0_instructed'],1)
...
c.session_training = 2; % session_training is 0 in the Learning version, where pts. 
                        ...
                        % 1 if already trained, 2 if they will JUST receive instructions here.
...
if c.scanner == 0
    ...
    preparestring(' ~ 响应 或 阻止 任务 ~ ', 0, 0, 160 );
...
%  Target practice
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if c.session==0 || c.session_training < 1  % Target practice if starting on learning version
...
if c.session_training == 2 % Only for the instructed, non-learning version !
       %% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
       % % Cues and Task Learning
       % %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
       preparestring('~ 响应-阻止任务的最佳反应 ~', c.InstructScreen,0,275);
       ...
       preparestring(['任务为按键赢 ' num2str(c.money) ' 元'],c.InstructScreen,0,-50);
       ...
       preparestring(['任务为按键以避免输 ' num2str(c.money) ' 元'],c.InstructScreen,0,-50);
       preparestring('... 准备好时请按任意键继续', c.InstructScreen,100,-275);
       drawpict(c.InstructScreen);
       waitkeydown(inf);
       clearpict(c.InstructScreen);
       ...
       preparestring(['任务为不要按键赢 ' num2str(c.money) ' 元'],c.InstructScreen,0,-50);
       preparestring('... 准备好时请按任意键继续', c.InstructScreen,100,-275);
   %     preparestring('...Press space-bar to continue', c.InstructScreen,250,-275);
       ...
       preparestring(['任务为不要按键以避免输 ' num2str(c.money) ' 元'],c.InstructScreen,0,-50);
       preparestring('... 准备好时请按任意键继续', c.InstructScreen,100,-275);
   %     preparestring('...Press space-bar to continue', c.InstructScreen,250,-275);
       drawpict(c.InstructScreen);
       waitkeydown(inf);
...
elseif c.session ~= 0 && c.scanner ~= 0 && c.session_training < 2 % scanner version, has sham trials
    ...
    preparestring('谢谢你的努力 - 做的很好!',c.InstructScreen,0,0);
    preparestring('请按任意键退出响应-阻止任务.',c.InstructScreen,0,-175);
    drawpict(c.InstructScreen);
    waitkeydown(inf);
    clearpict(c.InstructScreen);
