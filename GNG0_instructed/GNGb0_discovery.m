function TotalWon = GNGb0_discovery(code, dataDir, goNoGoDir, debug)
% function TotalWon = GNGi1(code, dataDir, goNoGoDir, debug)
% function for the Go-NoGo experiment after Marc Guitart. 
% the GNGb0_discovery version is a behavioural-only version to ACCOMPANY
% imaging (GNG *i* 1...) OUTSIDE the scanner.
%
% Commonly adjusted parameters: ( defaults shown here):
% c.NperCond = 20, c.cog_disp = 1, c.scanner=0 ...
% 
% Testing finalized code e.g. with 
% cd ( [getenv('HOMEDRIVE') getenv('HOMEPATH') '\Desktop\fMRI_REDIT\GNG\GNG_instructed_fMRI'] )
% GNGwinnings = GNGb0_discovery(['GNGid_' datestr(now,'yyyymmddHHMM')],[getenv('HOMEDRIVE') getenv('HOMEPATH') '\Dropbox\temp\'],'.',1)
...
if nargin < 4
    ...
    debug = 0; % 0 is default to just use the program, not test it. Use command
               % line above for testing finalized version.
end
if nargin < 3
  goNoGoDir = [getenv('HOMEDRIVE') getenv('HOMEPATH') '\Desktop\fMRI_REDIT\GNG\GNG_instructed_fMRI'] ;
  % [getenv('HOMEDRIVE') getenv('HOMEPATH') '\Dropbox\task_code\GNGb0_RewPun_fMRI'];
end
if nargin < 1
  code    = ['GNGb0' datestr(now,'yyyymmddHHMM')];
end
if nargin < 2
  dataDir = [getenv('HOMEDRIVE') getenv('HOMEPATH') '\Desktop\fMRI_REDIT\GNG\tempData\' code '_', datestr(now,'yyyy-mm-dd')];  
...
c.session_training = 0; % session_training is 0 in the Learning version, where pts. 
...
c.bad_dec_prob =1;   % If 1, BAD decisions get GOOD outcomes probabilistically with Py = 1-c.Psuccess 
                     ...
                     % outcomes. SEE NEXT LINE BEFORE CHANGING.
...
%c.tempNetDir      = ''; % Empty; for REDIT-BRAIN set to: '\\abba\stimuli\Task_repository\REDIT_fMRI\GNG\tempData';
%c.tempNetDir      = '\\abba\stimuli\Task_repository\REDIT_fMRI\GNG\tempData';
c.tempNetDir      = '\\ABBA\fMRI_REDIT\GNG\tempData';
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if c.session ~= 0 && c.scanner == 0 % Learning version, outside scanner
    % This is the bit that GNGb0_discovery uses outside scanner !
    TaskData = GNGb0_learning_task_display( c , ... % total trials = c.NperCond x 4 x 2
                                   xlocation1,xlocation2, ...
                                   LeftKey, RightKey );
    TotalWon=sum(TaskData(:,17));    
  
elseif c.session ~= 0 && c.scanner ~= 0 % scanner version, has sham trials
...
disp(' ');
disp(['GNGb0 discovery winnings: ',num2str(TotalWon),' (Go=',num2str(G),', NoGo=',num2str(NG),')']);
clear global c;

end % Of whole function GNGb0_discovery  *********************************
%*************************************************************************
